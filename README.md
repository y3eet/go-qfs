...
```powershell
Start-Process powershell -Verb RunAs -ArgumentList '-Command New-NetFirewallRule -DisplayName "Allow Port 8080 Private" -Direction Inbound -LocalPort 8080 -Protocol TCP -Action Allow -Profile Private'
```

**Close the port**

```powershell
Start-Process powershell -Verb RunAs -ArgumentList '-Command Remove-NetFirewallRule -DisplayName "Allow Port 8080 Private"'
```
...
_(Or use `sudo ufw deny 8080/tcp` if you want to explicitly block it rather than just removing the rule.)_
